...
class ConversionOrchestrator:
    ...
    def run_batch(self) -> None:
        logger.info("🚀 Starting Batch Conversion from html_text to bilara_more (Multithreaded)...")
        ...
        for html_file in lzh_dir.rglob("*.html"):
            ...
            tasks.append((html_file, rel_path))

        workers = min(os.cpu_count() or 4, 16)
        logger.info(f"   Using {workers} threads to process {len(tasks)} files...")

        with ThreadPoolExecutor(max_workers=workers) as executor:
            futures = {
                executor.submit(self.convert_file, html_path, rel_path): html_path.stem
                for html_path, rel_path in tasks
            }
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    logger.error(f"❌ Task failed for {uid}: {e}")
                    failed_count += 1

        logger.info(f"✨ Conversion completed! Converted: {converted_count}, Failed: {failed_count}")
